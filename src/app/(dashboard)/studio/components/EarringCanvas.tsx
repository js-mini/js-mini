// EarringCanvas — 2-slot upload grid + result/pending panel for "Küpe".
// JSX taken verbatim from studio-client.tsx lines 896-975.

import { GeneratedPanel, PendingPanel } from "./GeneratedPanel";
import { EARRING_SLOTS, type EarringSlot, type EarringFileState } from "./constants";
import { X } from "lucide-react";
import type { GenerateResult } from "@/lib/studio/actions";

type EarringCanvasProps = {
    earringFiles: EarringFileState;
    earringFileRefs: React.MutableRefObject<Record<EarringSlot, HTMLInputElement | null>>;
    isPending: boolean;
    statusText: string;
    result: GenerateResult;
    outputFormat: string;
    onFileChange: (slot: EarringSlot, file: File) => void;
    onClearSlot: (slot: EarringSlot) => void;
    onClearResult: () => void;
};
                ...
                <div className="w-full flex-1 flex flex-col items-center justify-center min-h-[300px] mt-4 mb-8 shrink-0">
                    {result.outputUrl ? (
                        <GeneratedPanel
                            ...
                            downloadFilename="earring_edited"
                            successLabel="Küpeler Üretildi"
                            onClose={onClearResult}
                            iconSize={14}
                            btnClass="w-8 h-8"
                        />
                    ) : isPending ? (
                        <PendingPanel statusText={statusText} fallbackText="Küpeler oluşturuluyor..." />
                    ) : null}
                </div>
            )}
        </div>
